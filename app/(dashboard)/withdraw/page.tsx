...
import { ChevronDown, AlertCircle, Check, ArrowLeft, Info, X, Loader2 } from "lucide-react";
  ...
  const handleCancelWithdrawal = async (txId: number) => {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (profile && data.new_balance) {
          setProfile({
            ...profile,
            balance: data.new_balance,
            formatted_balance: data.formatted_new_balance,
          });
        }
      } else {
                  ...
                  <tbody>
                    {transactions.map((transaction) => (
                      <React.Fragment key={transaction.id}>
                        <tr className="border-b border-slate-800 dark:border-slate-100 hover:bg-slate-800/50 dark:hover:bg-slate-50 transition-colors">
                          ...
                          <td className="py-5 px-4">
                            {transaction.status === "pending" && confirmCancelId !== transaction.id && (
                              <button
                        ...
                        {confirmCancelId === transaction.id && (
                          <tr className="border-b border-slate-800 dark:border-slate-100">
