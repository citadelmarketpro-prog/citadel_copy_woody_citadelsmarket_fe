...
const FAQItem = ({ question, content, isOpen, onToggle }: FAQItemProps) => (
  ...
  >
      ...
      >
        <span className={`font-medium text-lg pr-4 transition-colors ${isOpen ? "text-emerald-600" : "text-gray-900"}`}>
          {question}
        </span>
        {isOpen ? (
      ...
      <div
        className={`transition-all duration-500 ease-in-out overflow-hidden ${
          isOpen ? "max-h-[800px] opacity-100 px-6 pb-5" : "max-h-0 opacity-0 px-6 pb-0"
        }`}
      >
        <div className="space-y-3 border-t border-gray-200 pt-4">
          {content.map((block, i) => {
            ...
            if (block.type === "list") {
              return (
                <ul key={i} className="ml-4 space-y-1.5 text-sm text-gray-700">
            ...
            if (block.type === "ordered") {
              return (
                <ol key={i} className="ml-4 space-y-1.5 text-sm text-gray-700">
                  {block.items.map((item, j) => (
                    <li key={j} className="flex items-start gap-2">
